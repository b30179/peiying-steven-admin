...
-- Running upgrade 20260717_0007 -> 20260717_0008

INSERT INTO permissions (id, code, name, module) VALUES ('perm-tenders-read', 'steven:tenders:read', '读取标书与文书事项', 'steven');

INSERT INTO permissions (id, code, name, module) VALUES ('perm-tenders-write', 'steven:tenders:write', '创建及修订标书与文书事项', 'steven');

INSERT INTO permissions (id, code, name, module) VALUES ('perm-tenders-submit', 'steven:tenders:submit', '提交标书与文书审批', 'steven');

INSERT INTO permissions (id, code, name, module) VALUES ('perm-tenders-approve', 'steven:tenders:approve', '批准或退回标书与文书', 'steven');

INSERT INTO permissions (id, code, name, module) VALUES ('perm-tenders-export', 'steven:tenders:export', '导出已批准标书与文书', 'steven');

INSERT INTO permissions (id, code, name, module) VALUES ('perm-tenders-audit-scoped', 'steven:tenders:audit:read_scoped', '读取标书与文书事项审计', 'steven');

INSERT INTO role_permissions (role_id, permission_id) VALUES ('role-steven', 'perm-tenders-read');

INSERT INTO role_permissions (role_id, permission_id) VALUES ('role-steven', 'perm-tenders-write');

INSERT INTO role_permissions (role_id, permission_id) VALUES ('role-steven', 'perm-tenders-submit');

INSERT INTO role_permissions (role_id, permission_id) VALUES ('role-steven', 'perm-tenders-export');

INSERT INTO role_permissions (role_id, permission_id) VALUES ('role-steven', 'perm-tenders-audit-scoped');

INSERT INTO role_permissions (role_id, permission_id) VALUES ('role-approver', 'perm-tenders-read');

INSERT INTO role_permissions (role_id, permission_id) VALUES ('role-approver', 'perm-tenders-approve');

INSERT INTO role_permissions (role_id, permission_id) VALUES ('role-approver', 'perm-tenders-audit-scoped');

CREATE TABLE steven_templates (
    id VARCHAR(36) NOT NULL, 
    code VARCHAR(100) NOT NULL, 
    ...
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT now() NOT NULL, 
    PRIMARY KEY (id), 
    CONSTRAINT ck_steven_templates_version CHECK (version > 0), 
    CONSTRAINT ck_steven_templates_status CHECK (status IN ('active','inactive')), 
    CONSTRAINT uq_steven_templates_code_version UNIQUE (code, version), 
    FOREIGN KEY(created_by) REFERENCES users (id) ON DELETE SET NULL
);

CREATE INDEX ix_steven_templates_status_name ON steven_templates (status, name);

CREATE TABLE steven_tender_jobs (
    id VARCHAR(36) NOT NULL, 
    ...
    currency VARCHAR(3) DEFAULT 'HKD' NOT NULL, 
    location VARCHAR(300) NOT NULL, 
    controlled_clauses TEXT NOT NULL, 
    status VARCHAR(32) DEFAULT 'draft' NOT NULL, 
    rendered_body TEXT, 
    unresolved_variables JSONB DEFAULT '[]'::jsonb NOT NULL, 
    next_export_version INTEGER DEFAULT '1' NOT NULL, 
    submitted_by VARCHAR(36), 
    submitted_at TIMESTAMP WITH TIME ZONE, 
    decided_by VARCHAR(36), 
    decided_at TIMESTAMP WITH TIME ZONE, 
    decision_opinion TEXT, 
    ...
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT now() NOT NULL, 
    PRIMARY KEY (id), 
    CONSTRAINT ck_steven_tender_jobs_status CHECK (status IN ('draft','draft_error','submitted','approved','returned')), 
    CONSTRAINT ck_steven_tender_jobs_deadline CHECK (deadline_date >= generated_date + 3), 
    CONSTRAINT ck_steven_tender_jobs_budget CHECK (budget_min >= 0 AND budget_max >= 0 AND budget_min <= budget_max), 
    CONSTRAINT ck_steven_tender_jobs_currency CHECK (currency = upper(currency)), 
    CONSTRAINT ck_steven_tender_jobs_next_export_version CHECK (next_export_version > 0), 
    CONSTRAINT uq_steven_tender_jobs_document_number UNIQUE (document_number), 
    FOREIGN KEY(template_id) REFERENCES steven_templates (id) ON DELETE RESTRICT, 
...
CREATE INDEX ix_steven_tender_jobs_status_updated ON steven_tender_jobs (status, updated_at);

CREATE INDEX ix_steven_tender_jobs_template ON steven_tender_jobs (template_id);

CREATE TABLE steven_tender_suppliers (
    id VARCHAR(36) NOT NULL, 
    tender_job_id VARCHAR(36) NOT NULL, 
    supplier_name VARCHAR(250) NOT NULL, 
    normalized_name VARCHAR(250) NOT NULL, 
    created_by VARCHAR(36) NOT NULL, 
    created_at TIMESTAMP WITH TIME ZONE DEFAULT now() NOT NULL, 
    PRIMARY KEY (id), 
    CONSTRAINT uq_steven_tender_supplier_name UNIQUE (tender_job_id, normalized_name), 
    FOREIGN KEY(tender_job_id) REFERENCES steven_tender_jobs (id) ON DELETE CASCADE, 
    FOREIGN KEY(created_by) REFERENCES users (id) ON DELETE RESTRICT
);

CREATE INDEX ix_steven_tender_suppliers_job ON steven_tender_suppliers (tender_job_id, created_at);

CREATE TABLE steven_tender_versions (
    id VARCHAR(36) NOT NULL, 
    tender_job_id VARCHAR(36) NOT NULL, 
    file_id VARCHAR(36), 
    version_number INTEGER NOT NULL, 
    ...
    PRIMARY KEY (id), 
    CONSTRAINT ck_steven_tender_versions_number CHECK (version_number > 0), 
    CONSTRAINT ck_steven_tender_versions_status CHECK (status IN ('reserved','ready','failed')), 
    CONSTRAINT ck_steven_tender_versions_size CHECK (size_bytes IS NULL OR size_bytes >= 0), 
    CONSTRAINT uq_steven_tender_versions_job_version UNIQUE (tender_job_id, version_number), 
    CONSTRAINT uq_steven_tender_versions_storage_key UNIQUE (storage_key), 
    FOREIGN KEY(tender_job_id) REFERENCES steven_tender_jobs (id) ON DELETE RESTRICT, 
    FOREIGN KEY(file_id) REFERENCES files (id) ON DELETE RESTRICT, 
    FOREIGN KEY(created_by) REFERENCES users (id) ON DELETE RESTRICT
);

CREATE INDEX ix_steven_tender_versions_job_status ON steven_tender_versions (tender_job_id, status, version_number);

CREATE TABLE steven_tender_candidate_links (
    candidate_id VARCHAR(36) NOT NULL, 
    tender_job_id VARCHAR(36) NOT NULL, 
    candidate_kind VARCHAR(100) NOT NULL, 
    created_at TIMESTAMP WITH TIME ZONE DEFAULT now() NOT NULL, 
    PRIMARY KEY (candidate_id), 
    CONSTRAINT ck_steven_tender_candidate_kind CHECK (candidate_kind IN ('tender_source_extraction','clause_draft')), 
    FOREIGN KEY(candidate_id) REFERENCES review_candidates (id) ON DELETE RESTRICT, 
    FOREIGN KEY(tender_job_id) REFERENCES steven_tender_jobs (id) ON DELETE RESTRICT
);

CREATE INDEX ix_steven_tender_candidate_links_job ON steven_tender_candidate_links (tender_job_id, created_at);

UPDATE alembic_version SET version_num='20260717_0008' WHERE alembic_version.version_num = '20260717_0007';

-- Running upgrade 20260717_0008 -> 20260717_0009

INSERT INTO permissions (id, code, name, module) VALUES ('perm-inventory-read', 'steven:inventory:read', '读取消耗品库存与盘点', 'steven');

INSERT INTO permissions (id, code, name, module) VALUES ('perm-inventory-write', 'steven:inventory:write', '维护消耗品库存品项', 'steven');

INSERT INTO permissions (id, code, name, module) VALUES ('perm-inventory-count', 'steven:inventory:count', '创建及修订消耗品盘点', 'steven');

INSERT INTO permissions (id, code, name, module) VALUES ('perm-inventory-submit', 'steven:inventory:submit', '提交消耗品盘点审批', 'steven');

INSERT INTO permissions (id, code, name, module) VALUES ('perm-inventory-approve', 'steven:inventory:approve', '批准或退回消耗品盘点', 'steven');

INSERT INTO permissions (id, code, name, module) VALUES ('perm-inventory-export', 'steven:inventory:export', '导出已批准消耗品盘点', 'steven');

INSERT INTO permissions (id, code, name, module) VALUES ('perm-inventory-audit-scoped', 'steven:inventory:audit:read_scoped', '读取消耗品盘点事项审计', 'steven');

INSERT INTO role_permissions (role_id, permission_id) VALUES ('role-steven', 'perm-inventory-read');

INSERT INTO role_permissions (role_id, permission_id) VALUES ('role-steven', 'perm-inventory-write');

INSERT INTO role_permissions (role_id, permission_id) VALUES ('role-steven', 'perm-inventory-count');

INSERT INTO role_permissions (role_id, permission_id) VALUES ('role-steven', 'perm-inventory-submit');

INSERT INTO role_permissions (role_id, permission_id) VALUES ('role-steven', 'perm-inventory-export');

INSERT INTO role_permissions (role_id, permission_id) VALUES ('role-steven', 'perm-inventory-audit-scoped');

INSERT INTO role_permissions (role_id, permission_id) VALUES ('role-approver', 'perm-inventory-read');

INSERT INTO role_permissions (role_id, permission_id) VALUES ('role-approver', 'perm-inventory-approve');

INSERT INTO role_permissions (role_id, permission_id) VALUES ('role-approver', 'perm-inventory-audit-scoped');

CREATE TABLE steven_inventory_items (
    id VARCHAR(36) NOT NULL, 
    sku VARCHAR(120) NOT NULL, 
    normalized_sku VARCHAR(120) NOT NULL, 
    ...
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT now() NOT NULL, 
    PRIMARY KEY (id), 
    CONSTRAINT ck_steven_inventory_items_book_quantity CHECK (book_quantity >= 0), 
    CONSTRAINT ck_steven_inventory_items_safety_stock CHECK (safety_stock >= 0), 
    CONSTRAINT ck_steven_inventory_items_target_stock CHECK (target_stock >= safety_stock), 
    CONSTRAINT ck_steven_inventory_items_status CHECK (status IN ('active','inactive')), 
    CONSTRAINT uq_steven_inventory_items_normalized_sku UNIQUE (normalized_sku), 
    FOREIGN KEY(created_by) REFERENCES users (id) ON DELETE RESTRICT, 
    FOREIGN KEY(updated_by) REFERENCES users (id) ON DELETE RESTRICT
);

CREATE INDEX ix_steven_inventory_items_status_location ON steven_inventory_items (status, location, item_name);

CREATE TABLE steven_inventory_counts (
    id VARCHAR(36) NOT NULL, 
    ...
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT now() NOT NULL, 
    PRIMARY KEY (id), 
    CONSTRAINT ck_steven_inventory_counts_status CHECK (status IN ('draft','submitted','approved','returned')), 
    CONSTRAINT ck_steven_inventory_counts_next_export_version CHECK (next_export_version > 0), 
    CONSTRAINT uq_steven_inventory_counts_number UNIQUE (count_number), 
    FOREIGN KEY(submitted_by) REFERENCES users (id) ON DELETE RESTRICT, 
    FOREIGN KEY(decided_by) REFERENCES users (id) ON DELETE RESTRICT, 
    FOREIGN KEY(created_by) REFERENCES users (id) ON DELETE RESTRICT, 
    FOREIGN KEY(updated_by) REFERENCES users (id) ON DELETE RESTRICT
...
CREATE INDEX ix_steven_inventory_counts_status_updated ON steven_inventory_counts (status, updated_at);

CREATE TABLE steven_inventory_count_lines (
    id VARCHAR(36) NOT NULL, 
    inventory_count_id VARCHAR(36) NOT NULL, 
    inventory_item_id VARCHAR(36) NOT NULL, 
    sku_snapshot VARCHAR(120) NOT NULL, 
    item_name_snapshot VARCHAR(250) NOT NULL, 
    location_snapshot VARCHAR(200) NOT NULL, 
    book_quantity_snapshot INTEGER NOT NULL, 
    safety_stock_snapshot INTEGER NOT NULL, 
    target_stock_snapshot INTEGER NOT NULL, 
    counted_quantity INTEGER NOT NULL, 
    difference_quantity INTEGER NOT NULL, 
    is_low_stock BOOLEAN NOT NULL, 
    suggested_order_quantity INTEGER NOT NULL, 
    confirmed_order_quantity INTEGER NOT NULL, 
    manual_reason TEXT, 
    remark TEXT, 
    updated_by VARCHAR(36) NOT NULL, 
    created_at TIMESTAMP WITH TIME ZONE DEFAULT now() NOT NULL, 
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT now() NOT NULL, 
    PRIMARY KEY (id), 
    CONSTRAINT ck_steven_inventory_lines_book_quantity CHECK (book_quantity_snapshot >= 0), 
    CONSTRAINT ck_steven_inventory_lines_safety_stock CHECK (safety_stock_snapshot >= 0), 
    CONSTRAINT ck_steven_inventory_lines_target_stock CHECK (target_stock_snapshot >= safety_stock_snapshot), 
    CONSTRAINT ck_steven_inventory_lines_counted_quantity CHECK (counted_quantity >= 0), 
    CONSTRAINT ck_steven_inventory_lines_suggested_order CHECK (suggested_order_quantity >= 0), 
    CONSTRAINT ck_steven_inventory_lines_confirmed_order CHECK (confirmed_order_quantity >= 0), 
    CONSTRAINT ck_steven_inventory_lines_manual_reason CHECK (confirmed_order_quantity = suggested_order_quantity OR length(btrim(coalesce(manual_reason,''))) > 0), 
    CONSTRAINT uq_steven_inventory_count_item UNIQUE (inventory_count_id, inventory_item_id), 
    FOREIGN KEY(inventory_count_id) REFERENCES steven_inventory_counts (id) ON DELETE CASCADE, 
    FOREIGN KEY(inventory_item_id) REFERENCES steven_inventory_items (id) ON DELETE RESTRICT, 
    FOREIGN KEY(updated_by) REFERENCES users (id) ON DELETE RESTRICT
);

CREATE INDEX ix_steven_inventory_count_lines_count ON steven_inventory_count_lines (inventory_count_id, created_at);

CREATE INDEX ix_steven_inventory_count_lines_low_stock ON steven_inventory_count_lines (is_low_stock, inventory_item_id);

CREATE TABLE steven_inventory_versions (
    id VARCHAR(36) NOT NULL, 
    inventory_count_id VARCHAR(36) NOT NULL, 
    file_id VARCHAR(36), 
    version_number INTEGER NOT NULL, 
    ...
    published_at TIMESTAMP WITH TIME ZONE, 
    PRIMARY KEY (id), 
    CONSTRAINT ck_steven_inventory_versions_number CHECK (version_number > 0), 
    CONSTRAINT ck_steven_inventory_versions_status CHECK (status IN ('reserved','ready','failed')), 
    CONSTRAINT ck_steven_inventory_versions_size CHECK (size_bytes IS NULL OR size_bytes >= 0), 
    CONSTRAINT uq_steven_inventory_versions_count_version UNIQUE (inventory_count_id, version_number), 
    CONSTRAINT uq_steven_inventory_versions_storage_key UNIQUE (storage_key), 
    FOREIGN KEY(inventory_count_id) REFERENCES steven_inventory_counts (id) ON DELETE RESTRICT, 
    FOREIGN KEY(file_id) REFERENCES files (id) ON DELETE RESTRICT, 
    FOREIGN KEY(created_by) REFERENCES users (id) ON DELETE RESTRICT
);

CREATE INDEX ix_steven_inventory_versions_count_status ON steven_inventory_versions (inventory_count_id, status, version_number);

CREATE TABLE steven_inventory_candidate_links (
    candidate_id VARCHAR(36) NOT NULL, 
    inventory_count_id VARCHAR(36) NOT NULL, 
    candidate_kind VARCHAR(100) NOT NULL, 
    created_at TIMESTAMP WITH TIME ZONE DEFAULT now() NOT NULL, 
    PRIMARY KEY (candidate_id), 
    CONSTRAINT ck_steven_inventory_candidate_kind CHECK (candidate_kind IN ('inventory_sheet_extraction','inventory_exception_explanation')), 
    FOREIGN KEY(candidate_id) REFERENCES review_candidates (id) ON DELETE RESTRICT, 
    FOREIGN KEY(inventory_count_id) REFERENCES steven_inventory_counts (id) ON DELETE RESTRICT
);

CREATE INDEX ix_steven_inventory_candidate_links_count ON steven_inventory_candidate_links (inventory_count_id, created_at);

UPDATE alembic_version SET version_num='20260717_0009' WHERE alembic_version.version_num = '20260717_0008';
